...
with col1:
    ...
    hourly = filtered.groupby('hour').agg(value=(metric_col, agg_func)).reset_index()
...
with col2:
    ...
    bucket = filtered.groupby('time_bucket').agg(value=(metric_col, agg_func)).reset_index()
...
with col3:
    ...
    store_data = filtered.groupby('store_location').agg(value=(metric_col, agg_func)).reset_index()
...
with col4:
    ...
    cat_data = filtered.groupby('product_category').agg(value=(metric_col, agg_func)).reset_index().sort_values('value', ascending=True)
...
heatmap_data = df.groupby(['store_location', 'hour']).agg(value=(metric_col, agg_func)).reset_index()
